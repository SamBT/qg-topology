...
void samejet_sep_by_R(TString angularity, TString type, int rebin, int isleading, int groomed) {
  ...
  if (type == "qjet") {
    samp1 = {"dijets","zplusjets","H2qqbar"};
    samp2 = {"dijets","zplusjets","H2qqbar"};
    labels = {"Dijets","Z+jets","H #rightarrow q#bar{q}"};
  }
  else {
    samp1 = {"dijets","zplusjets","H2gg"};
    samp2 = {"dijets","zplusjets","H2gg"};
  ...
  for (int i = 0; i < samp1.size(); i++) {
    ...
    for (int j = 0; j < radii.size(); j++) {
      ...
      TFile *f2;
      if (groomed) {
	f1 = TFile::Open(analysis_path_reg+"/Radius_"+radii[j]+"/output_groomed_"+samp1[i]+"_rewt.root");
	f2 = TFile::Open(analysis_path_QCDa+"/Radius_"+radii[j]+"/output_groomed_"+samp2[i]+"_rewt.root");
      }
      else {
	f1 = TFile::Open(analysis_path_reg+"/Radius_"+radii[j]+"/output_"+samp1[i]+"_rewt.root");
	f2 = TFile::Open(analysis_path_QCDa+"/Radius_"+radii[j]+"/output_"+samp2[i]+"_rewt.root");
      }
  ...
  for (int l = 0; l < sep_graphs.size(); l++) {
    ...
    if (type == "qjet") sep_graphs[l]->GetYaxis()->SetRangeUser(0,0.13);
    if (type == "gjet") sep_graphs[l]->GetYaxis()->SetRangeUser(0,0.13);
  ...
  l.SetNDC();
  l.SetTextSize(0.04);
  l.DrawLatex(0.5,0.87,"PYTHIA 8.2");
  l.SetTextSize(0.03);
  ...
  l.DrawLatex(0.5,0.96,"#bf{Default vs. QCD-aware Labeling}");
