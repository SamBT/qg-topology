...
void kappa_beta_samejet_sep(TString radius, TString type, int rebin, int isleading, int groomed) {
  ...
  if (type == "qjet") {
    samp1 = {"dijets","zplusjets"};
    samp2 = {"dijets","zplusjets"};
    labels = {"Dijets","Z+jets"};
  }
  else {
    samp1 = {"dijets","zplusjets"};
    samp2 = {"dijets","zplusjets"};
    labels = {"Dijets","Z+jets","H #rightarrow gg"};
  }
  ...
  for (int i = 0; i < samp1.size(); i++) {
    ...
    TFile *f2;
    if (groomed) {
      f1 = TFile::Open(analysis_path_reg+"/Radius_"+radius+"/output_groomed_"+samp1[i]+"_rewt.root");
      f2 = TFile::Open(analysis_path_QCDa+"/Radius_"+radius+"/output_groomed_"+samp2[i]+"_rewt.root");
    }
    else {
      f1 = TFile::Open(analysis_path_reg+"/Radius_"+radius+"/output_"+samp1[i]+"_rewt.root");
      f2 = TFile::Open(analysis_path_QCDa+"/Radius_"+radius+"/output_"+samp2[i]+"_rewt.root");
    }
  ...
  TLegend leg(0.60,0.75,0.80,0.9);
  leg.SetLineColor(0);
  leg.SetFillStyle(0);    
  ...
  for (int l = 0; l < sep_hists.size(); l++) {
    ...
    sep_hists[l]->GetYaxis()->SetRangeUser(0,0.06);
    sep_hists[l]->SetLabelSize(0.04,"y");
    sep_hists[l]->GetYaxis()->SetTitleOffset(1.5);
    //sep_hists[l]->SetTitleSize(0.04,"y");
    sep_hists[l]->SetLineWidth(2);
  ...
  l.SetTextSize(0.03);
  if (type == "qjet") message = "Quark Jets, "+rad_str;
  if (type == "gjet") message = "Gluon Jets, "+rad_str;
  if (groomed) {l.DrawLatex(0.2,0.82,"Groomed "+message);} else {l.DrawLatex(0.2,0.82,message);}
  l.DrawLatex(0.2,0.96,"#bf{Default vs. QCD-aware Labeling}");
