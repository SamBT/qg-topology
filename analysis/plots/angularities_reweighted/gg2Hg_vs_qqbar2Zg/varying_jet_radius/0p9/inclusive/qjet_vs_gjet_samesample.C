void qjet_vs_gjet_samesample(TString type, TString var, TString xlabel, int rebin, int logscale) {
  TString filename;
  if (type == "gg2Hg") filename = "/global/homes/s/sambt/Jets/qg_tagging_topology_project/analysis/analysis_output/Radius_09/output_gg2Hg_rewt.root";
  if (type == "qqbar2Zg") filename = "/global/homes/s/sambt/Jets/qg_tagging_topology_project/analysis/analysis_output/Radius_09/output_qqbar2Zg_rewt.root";
  
  TFile *f1 = TFile::Open(filename);

  TH1F *hq = (TH1F*)f1->Get("h_qjet_leading_"+var);
  TH1F *hg = (TH1F*)f1->Get("h_gjet_leading_"+var);
  ...
  l.SetNDC();
  l.SetTextSize(0.04);
  if (type == "gg2Hg") l.DrawLatex(0.45,0.85,"gg #rightarrow Hg, p_{T}^{jet} > 20 GeV");
  if (type == "qqbar2Zg") l.DrawLatex(0.45,0.85,"q#bar{q} #rightarrow Zg, p_{T}^{jet} > 20 GeV");
  l.DrawLatex(0.45,0.8,sep);
  l.DrawLatex(0.45,0.75,"R = 0.9");
