void samejet_diffsample(TString type, TString var, TString xlabel, int rebin, int logscale) {
  TFile *f1 = TFile::Open("/global/homes/s/sambt/Jets/qg_tagging_topology_project/analysis/analysis_output/Radius_08/output_gg2Hg_rewt.root");
  TFile *f2 = TFile::Open("/global/homes/s/sambt/Jets/qg_tagging_topology_project/analysis/analysis_output/Radius_08/output_qqbar2Zg_rewt.root");

  TH1F *hd = (TH1F*)f1->Get("h_"+type+"_leading_"+var);
  TH1F *hz = (TH1F*)f2->Get("h_"+type+"_leading_"+var);
  ...
  leg.AddEntry(hd,"gg #rightarrow Hg","l");
  leg.AddEntry(hz,"q#bar{q} #rightarrow Zg","l");
  leg.Draw();
  TLatex l;
  l.SetNDC();
  l.SetTextSize(0.04);
  ...
  l.DrawLatex(0.45,0.8,"Reweighted to Z+jets in p_{T} and #eta");
  l.DrawLatex(0.45,0.75,sep);
  l.DrawLatex(0.45,0.7,"R = 0.8");
