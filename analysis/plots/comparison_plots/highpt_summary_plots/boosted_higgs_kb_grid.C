#include "helper.hh"
void boosted_higgs_kb_grid(TString radius, TString type, int rebin, int isleading, int groomed) {
  vector<TString> samp1;
  vector<TString> samp2;
  vector<TString> labels;
  if (type == "qjet") {
    samp1 = {"dijets","dijets","zplusjets","zplusjets","H2qqbar"};
    samp2 = {"H2qqbar","H2qqbar_boosted","H2qqbar","H2qqbar_boosted","H2qqbar_boosted"};
  }
  else {
    samp1 = {"dijets","dijets","zplusjets","zplusjets","H2gg"};
    samp2 = {"H2gg","H2gg_boosted","H2gg","H2gg_boosted","H2gg_boosted"};
  }
  ...
  for (int i = 0; i < samp1.size(); i++) {
    ...
    if (i+1 == 5) {seps->SetLineColor(6);} else {seps->SetLineColor(i+1);}
