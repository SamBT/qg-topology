...
void samejet_sep_by_R(TString angularity, TString type, int rebin, int isleading, int groomed) {
  ...
  for (int l = 0; l < sep_graphs.size(); l++) {
    ...
    if (type == "qjet") sep_graphs[l]->GetYaxis()->SetRangeUser(0,0.18);
    if (type == "gjet") sep_graphs[l]->GetYaxis()->SetRangeUser(0,0.18);
    sep_graphs[l]->GetYaxis()->SetNdivisions(505);
    sep_graphs[l]->GetYaxis()->SetTitleOffset(1.8);
  ...
  l.DrawLatex(0.5,0.78,"QCD aware");
