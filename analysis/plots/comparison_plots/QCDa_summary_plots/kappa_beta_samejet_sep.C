...
void kappa_beta_samejet_sep(TString radius, TString type, int rebin, int isleading, int groomed) {
  ...
  for (int l = 0; l < sep_hists.size(); l++) {
    ...
    if (type == "qjet") sep_hists[l]->GetYaxis()->SetRangeUser(0,0.15);
    if (type == "gjet") sep_hists[l]->GetYaxis()->SetRangeUser(0,0.15);
  ...
  //l.DrawLatex(0.16,0.96,"QCD-aware flavor tagging");
