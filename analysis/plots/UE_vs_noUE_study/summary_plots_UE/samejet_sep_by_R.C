...
void samejet_sep_by_R(TString angularity, TString type, int rebin, int isleading, int groomed) {
  ...
  if (type == "qjet") {
    ...
    samp1 = {"dijets","dijets","zplusjets"};
    samp2 = {"zplusjets","H2gg","H2gg"};
    labels = {"Dijets vs Z+jets","Dijets vs H #rightarrow gg","Z+jets vs H #rightarrow gg"};
  }
  ...
  for (int l = 0; l < sep_graphs.size(); l++) {
    ...
    if (type == "qjet") sep_graphs[l]->GetYaxis()->SetRangeUser(0,0.03);
    if (type == "gjet") sep_graphs[l]->GetYaxis()->SetRangeUser(0,0.03);
