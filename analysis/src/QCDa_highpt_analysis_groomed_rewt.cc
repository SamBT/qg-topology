...
int main (int argc, char **argv) {
  ...
  for( int i = 0 ; i < numev ; i ++ ) {	
    ...
    for (int n = 0; n < max_index; n++) {
      ...
      if (nqjets_filled+1 <= p->QCDaNQjetsSmallR && p->QCDaQjetPtSmallR->at(nqjets_filled) == p->JsmallPt->at(n)) {
	wt_pt = qjet_pt_wts->GetBinContent(qjet_pt_denom->GetXaxis()->FindBin(p->QCDaQjetPtSmallR->at(nqjets_filled)));
	wt_eta = qjet_eta_wts->GetBinContent(qjet_eta_denom->GetXaxis()->FindBin(p->QCDaQjetEtaSmallR->at(nqjets_filled)));
	wt = wt_pt*wt_eta;
	
	h_qjet_pt->Fill(p->QCDaQjetPtSmallR_SD->at(nqjets_filled),wt_pt);
        h_qjet_eta->Fill(p->QCDaQjetEtaSmallR_SD->at(nqjets_filled),wt_eta);
        h_qjet_phi->Fill(p->QCDaQjetPhiSmallR_SD->at(nqjets_filled),wt);
        h_qjet_M->Fill(p->QCDaQjetMSmallR_SD->at(nqjets_filled),wt);
        h_qjet_multiplicity->Fill(p->QCDaQjetMultSmallR_SD->at(nqjets_filled),wt);
        h_qjet_ptD->Fill(p->QCDaQjetpTDSmallR_SD->at(nqjets_filled),wt);
        h_qjet_LHA->Fill(p->QCDaQjetLHASmallR_SD->at(nqjets_filled),wt);
        h_qjet_width->Fill(p->QCDaQjetWidthSmallR_SD->at(nqjets_filled),wt);
        h_qjet_mass->Fill(p->QCDaQjetMassSmallR_SD->at(nqjets_filled),wt);
	nqjets_filled += 1;
      }
      if (ngjets_filled+1 <= p->QCDaNGjetsSmallR && p->QCDaGjetPtSmallR->at(ngjets_filled) == p->JsmallPt->at(n)) {
	wt_pt = gjet_pt_wts->GetBinContent(gjet_pt_denom->GetXaxis()->FindBin(p->QCDaGjetPtSmallR->at(ngjets_filled)));
	wt_eta = gjet_eta_wts->GetBinContent(gjet_eta_denom->GetXaxis()->FindBin(p->QCDaGjetEtaSmallR->at(ngjets_filled)));
	wt = wt_pt*wt_eta;

	h_gjet_pt->Fill(p->QCDaGjetPtSmallR_SD->at(ngjets_filled),wt_pt);
        h_gjet_eta->Fill(p->QCDaGjetEtaSmallR_SD->at(ngjets_filled),wt_eta);
        h_gjet_phi->Fill(p->QCDaGjetPhiSmallR_SD->at(ngjets_filled),wt);
        h_gjet_M->Fill(p->QCDaGjetMSmallR_SD->at(ngjets_filled),wt);
        h_gjet_multiplicity->Fill(p->QCDaGjetMultSmallR_SD->at(ngjets_filled),wt);
        h_gjet_ptD->Fill(p->QCDaGjetpTDSmallR_SD->at(ngjets_filled),wt);
        h_gjet_LHA->Fill(p->QCDaGjetLHASmallR_SD->at(ngjets_filled),wt);
        h_gjet_width->Fill(p->QCDaGjetWidthSmallR_SD->at(ngjets_filled),wt);
        h_gjet_mass->Fill(p->QCDaGjetMassSmallR_SD->at(ngjets_filled),wt);
	ngjets_filled += 1;
      }
    }
