...
// Constructor 
FCNCAnalysis::FCNCAnalysis(double jet_radius){
...
// Analyze
void FCNCAnalysis::AnalyzeEvent(int ievt, TChain *chain) {

  p = new hepmc3_tree(chain);
  get_event(ievt);
  vector<int> pid(p->particles_pid,p->particles_pid+p->particles_);
  vector<int> status(p->particles_status,p->particles_status+p->particles_);
  vector<double> px(p->particles_momentum_m_v1,p->particles_momentum_m_v1+p->particles_);
  vector<double> py(p->particles_momentum_m_v2,p->particles_momentum_m_v2+p->particles_);
  vector<double> pz(p->particles_momentum_m_v3,p->particles_momentum_m_v3+p->particles_);
  vector<double> e(p->particles_momentum_m_v4,p->particles_momentum_m_v4+p->particles_);
  ...
  int HiggsIndex = -999;
  int daughter_id;
  double min_jet_pt = 20;
  
  // Particle loop -----------------------------------------------------------
  for (unsigned int ip=0; ip < p->particles_; ++ip){
    
    fastjet::PseudoJet p(px[ip],py[ip],pz[ip],e[ip]);
    p.set_user_info(new MyUserInfo(pid[ip],ip,-1));
    p.set_user_index(pid[ip]);
    ...
    */
    // particles for jets --------------
    if (status[ip] != 1 )      continue;
    if (fabs(pid[ip])  ==12) continue;
    if (fabs(pid[ip])  ==13) continue;
    if (fabs(pid[ip])  ==14) continue;
    if (fabs(pid[ip])  ==16) continue;
  ...
  */
  ...
  for (unsigned int ij = 0; ij < myJets.size(); ij++){
    ...
    if (!HiggsTagger) {
      int index = -1;
      for (unsigned int k = 0; k < p->particles_; k++) {
	...
	loop_parton = fastjet::PseudoJet(px[k],py[k],pz[k],e[k]);
	...
	if (dR < m_jet_def->R() && loop_parton.e() > E_parton && loop_parton.e() > min_parton_E) {
	  ...
	  if (parton_id != 21 && parton_id != 9) {
	    if (parton_id == 4 || parton_id == 5 || parton_id == 6) {
	      iscbt = true;
	    }
	    else {
	      iscbt = false;
    ...
    */
    ...
    if (matched) {
      if (fabs(matched_id) <= 6 && matched_id != 0) {
