...
fastjet::PseudoJet FCNCTools::Add(fastjet::PseudoJet jet){
...
fastjet::PseudoJet FCNCTools::Angles(fastjet::PseudoJet jet){
  fastjet::PseudoJet out(0.,0.,0.,0.);
  TRandom3 *rand = new TRandom3(0);
...
fastjet::PseudoJet FCNCTools::Drop(fastjet::PseudoJet jet){
  fastjet::PseudoJet out(0.,0.,0.,0.);
  TRandom3 *rand = new TRandom3(0);
  for (unsigned int i=0; i<jet.constituents().size(); i++){
...
fastjet::PseudoJet FCNCTools::Scale(fastjet::PseudoJet jet){
  fastjet::PseudoJet out(0.,0.,0.,0.);
  TRandom3 *rand = new TRandom3(0);
  for (unsigned int i=0; i<jet.constituents().size(); i++){
...
double FCNCTools::width(fastjet::PseudoJet jet){
  double out=0.;
  for (unsigned int i=0; i<jet.constituents().size(); i++){
...
int FCNCTools::ntrack(fastjet::PseudoJet jet,double pT){
  int out=0.;
  for (unsigned int i=0; i<jet.constituents().size(); i++){
...
double FCNCTools::JetTrackMass(fastjet::PseudoJet jet, int which){
...
double FCNCTools::JetCharge(fastjet::PseudoJet jet,double kappa){
...
bool FCNCTools::IsBHadron(int pdgId){
...
bool FCNCTools::IsCHadron(int pdgId){
  int abs_pdgId = abs(pdgId);
  int abs_pdgId_mod10k = (abs(pdgId)%10000);
...
bool FCNCTools::Btag(fastjet::PseudoJet jet,vector<fastjet::PseudoJet> bhadrons,vector<fastjet::PseudoJet> chadrons,double jetrad,double b,double c,double uds){
...
bool FCNCTools::BosonMatch(fastjet::PseudoJet jet, vector<fastjet::PseudoJet> Bosons, double jetrad, int BosonID ){
...
bool FCNCTools::IsIsolated(Pythia8::Particle* particle, Pythia8::Pythia* pythia8, float rel_iso, float ConeSize){
    float sumpT=0;
