...
int main (int argc, char **argv) {
  ...
  for( int i = 0 ; i < numev ; i ++ ) {	
    ...
    for (int k = 0; k < p->QCDaNQjetsSmallR; k++) {
      if (p->QCDaQjetPtSmallR->at(k) < 200 || p->QCDaQjetPtSmallR->at(k) > 400 || fabs(p->QCDaQjetEtaSmallR->at(k)) > 2.0) continue;
	...
	if (p->QCDaQjetEtaSmallR->at(k) > qjet_eta_denom->GetBinLowEdge(r) && p->QCDaQjetEtaSmallR->at(k) < qjet_eta_denom->GetBinLowEdge(r) + qjet_eta_denom->GetBinWidth(r)) {
      ...
      if (k == 0) {
	h_qjet_leading_pt->Fill(p->QCDaQjetPtSmallR_SD->at(k),leading_wt_pt);
	h_qjet_leading_eta->Fill(p->QCDaQjetEtaSmallR_SD->at(k),leading_wt_eta);
	h_qjet_leading_multiplicity->Fill(p->QCDaQjetMultSmallR_SD->at(k), leading_wt);
	h_qjet_leading_ptD->Fill(p->QCDaQjetpTDSmallR_SD->at(k),leading_wt);
	h_qjet_leading_LHA->Fill(p->QCDaQjetLHASmallR_SD->at(k),leading_wt);
	h_qjet_leading_width->Fill(p->QCDaQjetWidthSmallR_SD->at(k),leading_wt);
	h_qjet_leading_mass->Fill(p->QCDaQjetMassSmallR_SD->at(k),leading_wt);
      }
      if (k == 1) {
	h_qjet_subleading_pt->Fill(p->QCDaQjetPtSmallR_SD->at(k),subleading_wt_pt);
	h_qjet_subleading_eta->Fill(p->QCDaQjetEtaSmallR_SD->at(k),subleading_wt_eta);
	h_qjet_subleading_multiplicity->Fill(p->QCDaQjetMultSmallR_SD->at(k),subleading_wt);
	h_qjet_subleading_ptD->Fill(p->QCDaQjetpTDSmallR_SD->at(k),subleading_wt);
	h_qjet_subleading_LHA->Fill(p->QCDaQjetLHASmallR_SD->at(k),subleading_wt);
	h_qjet_subleading_width->Fill(p->QCDaQjetWidthSmallR_SD->at(k),subleading_wt);
	h_qjet_subleading_mass->Fill(p->QCDaQjetMassSmallR_SD->at(k),subleading_wt);
      }

      h_qjet_pt->Fill(p->QCDaQjetPtSmallR_SD->at(k),wt_pt);
      h_qjet_eta->Fill(p->QCDaQjetEtaSmallR_SD->at(k),wt_eta);
      h_qjet_phi->Fill(p->QCDaQjetPhiSmallR_SD->at(k),wt);
      h_qjet_M->Fill(p->QCDaQjetMSmallR_SD->at(k),wt);
      h_qjet_multiplicity->Fill(p->QCDaQjetMultSmallR_SD->at(k), wt);
      h_qjet_ptD->Fill(p->QCDaQjetpTDSmallR_SD->at(k),wt);
      h_qjet_LHA->Fill(p->QCDaQjetLHASmallR_SD->at(k),wt);
      h_qjet_width->Fill(p->QCDaQjetWidthSmallR_SD->at(k),wt);
      h_qjet_mass->Fill(p->QCDaQjetMassSmallR_SD->at(k),wt);

      h_qjet_multiplicity_eta->Fill(p->QCDaQjetMultSmallR_SD->at(k),p->QCDaQjetEtaSmallR_SD->at(k),wt);
      h_qjet_ptD_eta->Fill(p->QCDaQjetpTDSmallR_SD->at(k),p->QCDaQjetEtaSmallR_SD->at(k),wt);
      h_qjet_LHA_eta->Fill(p->QCDaQjetLHASmallR_SD->at(k),p->QCDaQjetEtaSmallR_SD->at(k),wt);
      h_qjet_width_eta->Fill(p->QCDaQjetWidthSmallR_SD->at(k),p->QCDaQjetEtaSmallR_SD->at(k),wt);
      h_qjet_mass_eta->Fill(p->QCDaQjetMassSmallR_SD->at(k),p->QCDaQjetEtaSmallR_SD->at(k),wt);

      eta = p->QCDaQjetEtaSmallR_SD->at(k);
      phi = p->QCDaQjetPhiSmallR_SD->at(k);
      dRmin = 999999;
	...
	if (p->JsmallPt->at(l) < 20) continue;
      ...
      h_qjet_multiplicity_nearestdR->Fill(p->QCDaQjetMultSmallR_SD->at(k),dRmin,wt);
      h_qjet_ptD_nearestdR->Fill(p->QCDaQjetpTDSmallR_SD->at(k),dRmin,wt);
      h_qjet_LHA_nearestdR->Fill(p->QCDaQjetLHASmallR_SD->at(k),dRmin,wt);
      h_qjet_width_nearestdR->Fill(p->QCDaQjetWidthSmallR_SD->at(k),dRmin,wt);
      h_qjet_mass_nearestdR->Fill(p->QCDaQjetMassSmallR_SD->at(k),dRmin,wt);
    }

    for (int j = 0; j < p->QCDaNGjetsSmallR; j++) {
      if (p->QCDaGjetPtSmallR->at(j) < 200 || p->QCDaGjetPtSmallR->at(j) > 400 || fabs(p->QCDaGjetEtaSmallR->at(j)) > 2.0) continue;
      ...
      if (j == 0) {
	h_gjet_leading_pt->Fill(p->QCDaGjetPtSmallR_SD->at(j),wt_pt);
        h_gjet_leading_eta->Fill(p->QCDaGjetEtaSmallR_SD->at(j),wt_eta);
        h_gjet_leading_multiplicity->Fill(p->QCDaGjetMultSmallR_SD->at(j), wt);
        h_gjet_leading_ptD->Fill(p->QCDaGjetpTDSmallR_SD->at(j),wt);
        h_gjet_leading_LHA->Fill(p->QCDaGjetLHASmallR_SD->at(j),wt);
        h_gjet_leading_width->Fill(p->QCDaGjetWidthSmallR_SD->at(j),wt);
        h_gjet_leading_mass->Fill(p->QCDaGjetMassSmallR_SD->at(j),wt);
      } 
      if (j == 1) {
	h_gjet_subleading_pt->Fill(p->QCDaGjetPtSmallR_SD->at(j),wt_pt);
        h_gjet_subleading_eta->Fill(p->QCDaGjetEtaSmallR_SD->at(j),wt_eta);
        h_gjet_subleading_multiplicity->Fill(p->QCDaGjetMultSmallR_SD->at(j),wt);
        h_gjet_subleading_ptD->Fill(p->QCDaGjetpTDSmallR_SD->at(j),wt);
        h_gjet_subleading_LHA->Fill(p->QCDaGjetLHASmallR_SD->at(j),wt);
        h_gjet_subleading_width->Fill(p->QCDaGjetWidthSmallR_SD->at(j),wt);
        h_gjet_subleading_mass->Fill(p->QCDaGjetMassSmallR_SD->at(j),wt);
      }

      h_gjet_pt->Fill(p->QCDaGjetPtSmallR_SD->at(j),wt_pt);
      h_gjet_eta->Fill(p->QCDaGjetEtaSmallR_SD->at(j),wt_eta);
      h_gjet_phi->Fill(p->QCDaGjetPhiSmallR_SD->at(j),wt);
      h_gjet_M->Fill(p->QCDaGjetMSmallR_SD->at(j),wt);
      h_gjet_multiplicity->Fill(p->QCDaGjetMultSmallR_SD->at(j),wt);
      h_gjet_ptD->Fill(p->QCDaGjetpTDSmallR_SD->at(j),wt);
      h_gjet_LHA->Fill(p->QCDaGjetLHASmallR_SD->at(j),wt);
      h_gjet_width->Fill(p->QCDaGjetWidthSmallR_SD->at(j),wt);
      h_gjet_mass->Fill(p->QCDaGjetMassSmallR_SD->at(j),wt);

      h_gjet_multiplicity_eta->Fill(p->QCDaGjetMultSmallR_SD->at(j),p->QCDaGjetEtaSmallR_SD->at(j),wt);
      h_gjet_ptD_eta->Fill(p->QCDaGjetpTDSmallR_SD->at(j),p->QCDaGjetEtaSmallR_SD->at(j),wt);
      h_gjet_LHA_eta->Fill(p->QCDaGjetLHASmallR_SD->at(j),p->QCDaGjetEtaSmallR_SD->at(j),wt);
      h_gjet_width_eta->Fill(p->QCDaGjetWidthSmallR_SD->at(j),p->QCDaGjetEtaSmallR_SD->at(j),wt);
      h_gjet_mass_eta->Fill(p->QCDaGjetMassSmallR_SD->at(j),p->QCDaGjetEtaSmallR_SD->at(j),wt);

      eta = p->QCDaGjetEtaSmallR_SD->at(j);
      phi = p->QCDaGjetPhiSmallR_SD->at(j);
      dRmin = 999999;
	...
	if (p->JsmallPt->at(l) < 20) continue;
      ...
      h_gjet_multiplicity_nearestdR->Fill(p->QCDaGjetMultSmallR_SD->at(j),dRmin,wt);
      h_gjet_ptD_nearestdR->Fill(p->QCDaGjetpTDSmallR_SD->at(j),dRmin,wt);
      h_gjet_LHA_nearestdR->Fill(p->QCDaGjetLHASmallR_SD->at(j),dRmin,wt);
      h_gjet_width_nearestdR->Fill(p->QCDaGjetWidthSmallR_SD->at(j),dRmin,wt);
      h_gjet_mass_nearestdR->Fill(p->QCDaGjetMassSmallR_SD->at(j),dRmin,wt);
    }
