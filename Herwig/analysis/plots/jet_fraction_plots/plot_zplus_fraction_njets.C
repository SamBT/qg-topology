void plot_zplus_fraction_njets(int njets) {
  TString rootname; rootname.Form("output_zplusjets_%ijets.root",njets);
  ...
  if (njets == 1) {
    ...
    q->Draw("lpfsame");
    TLatex l;
    l.DrawLatex(20,1.1,"Z+1 jets");
    l.DrawLatex(pt[3],y_q[3] - 0.5*y_q[3],"Q");
    l.DrawLatex(pt[3],y_q[3]+0.5*(1-y_q[3]),"G");
    c->SaveAs("fractions_zplus1jets.png");
    c->SaveAs("fractions_zplus1jets.pdf");
  }
  if (njets == 2) {
  ...
  if (njets == 3) {
    ...
    c->SaveAs("fractions_zplus3jets.png");
    c->SaveAs("fractions_zplus3jets.pdf");
  }
}
