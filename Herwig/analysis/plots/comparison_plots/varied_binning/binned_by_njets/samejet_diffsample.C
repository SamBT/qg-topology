void samejet_diffsample(TString type, TString var, TString xlabel, int rebin, int logscale, int njets) {
  TString fname1; fname1.Form("/global/homes/s/sambt/Jets/qg_tagging_topology_project/analysis/analysis_output/Radius_04/output_dijets_rewt_%ijets.root",njets);
  TString fname2; fname2.Form("/global/homes/s/sambt/Jets/qg_tagging_topology_project/analysis/analysis_output/Radius_04/output_zplusjets_%ijets.root",njets-1);
  ...
  TString dijets_label; dijets_label.Form("Dijets, %i jets",njets);
  TString zplusjets_label; zplusjets_label.Form("Z + %i jets",njets-1);
  leg.AddEntry(hd,dijets_label,"l");
  leg.AddEntry(hz,zplusjets_label,"l");
  leg.Draw();
  TLatex l;
  l.SetNDC();
  l.SetTextSize(0.04);
