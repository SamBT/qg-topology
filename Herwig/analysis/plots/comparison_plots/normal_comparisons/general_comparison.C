...
void general_comparison(TString sample1, TString sample2, TString type1, TString type2, TString var, int isleading, TString radius, int rebin) {
  ...
  TString file1 = "/global/homes/s/sambt/Jets/qg_tagging_topology_project/analysis/analysis_output/Radius_"+radius+"/output_"+sample1+".root";
  TString file2 = "/global/homes/s/sambt/Jets/qg_tagging_topology_project/analysis/analysis_output/Radius_"+radius+"/output_"+sample2+".root";
  ...
  l.SetNDC();
  l.SetTextSize(0.04);
  l.DrawLatex(0.55,0.85,"p_{T}^{jet} > 20 GeV, "+rad_str);
