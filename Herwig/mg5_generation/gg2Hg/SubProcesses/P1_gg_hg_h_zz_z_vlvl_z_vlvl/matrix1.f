...
C     *   Decay: h > z z HIG<=1 HIW<=1 WEIGHTED<=2
C     *     Decay: z > ve ve~ HIG<=1 HIW<=1 WEIGHTED<=2
C     *     Decay: z > ve ve~ HIG<=1 HIW<=1 WEIGHTED<=2
C     Process: g g > h g HIG<=1 HIW<=1 WEIGHTED<=3 @1
C     *   Decay: h > z z HIG<=1 HIW<=1 WEIGHTED<=2
C     *     Decay: z > vm vm~ HIG<=1 HIW<=1 WEIGHTED<=2
C     *     Decay: z > vm vm~ HIG<=1 HIW<=1 WEIGHTED<=2
C     Process: g g > h g HIG<=1 HIW<=1 WEIGHTED<=3 @1
C     *   Decay: h > z z HIG<=1 HIW<=1 WEIGHTED<=2
C     *     Decay: z > vt vt~ HIG<=1 HIW<=1 WEIGHTED<=2
C     *     Decay: z > vt vt~ HIG<=1 HIW<=1 WEIGHTED<=2
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
...
C     ----------
      ...
      CALL FFV2_3(W(1,4),W(1,3),GC_59,MDL_MZ,MDL_WZ,W(1,5))
      CALL OXXXXX(P(0,5),ZERO,NHEL(5),+1*IC(5),W(1,4))
      CALL IXXXXX(P(0,6),ZERO,NHEL(6),-1*IC(6),W(1,3))
      CALL FFV2_3(W(1,3),W(1,4),GC_59,MDL_MZ,MDL_WZ,W(1,6))
      CALL VVS2_3(W(1,5),W(1,6),GC_73,MDL_MH,MDL_WH,W(1,3))
      CALL VXXXXX(P(0,7),ZERO,NHEL(7),+1*IC(7),W(1,6))
C     Amplitude(s) for diagram number 1
      CALL VVVS2_0(W(1,1),W(1,2),W(1,6),W(1,3),GC_14,AMP(1))
      CALL VVV1P0_1(W(1,1),W(1,2),GC_10,ZERO,ZERO,W(1,5))
C     Amplitude(s) for diagram number 2
      CALL VVS3_0(W(1,6),W(1,5),W(1,3),GC_13,AMP(2))
      CALL VVS3P0_1(W(1,1),W(1,3),GC_13,ZERO,ZERO,W(1,5))
C     Amplitude(s) for diagram number 3
      CALL VVV1_0(W(1,2),W(1,6),W(1,5),GC_10,AMP(3))
      CALL VVV1P0_1(W(1,1),W(1,6),GC_10,ZERO,ZERO,W(1,5))
C     Amplitude(s) for diagram number 4
      CALL VVS3_0(W(1,2),W(1,5),W(1,3),GC_13,AMP(4))
C     JAMPs contributing to orders ALL_ORDERS=1
      JAMP(1,1)=+2D0*(-IMAG1*AMP(1)-IMAG1*AMP(2)-IMAG1*AMP(3)+IMAG1
     $ *AMP(4))
      JAMP(2,1)=+2D0*(+IMAG1*AMP(1)+IMAG1*AMP(2)+IMAG1*AMP(3)-IMAG1
     $ *AMP(4))
